...
export function TodayDashboard({ plan, reasons, workouts, preferences, gyms, context, inProgress, loggedSets, workload, onContextChange, onStart, onPlans, onHistory, children }: Props) {
        ...
        <section className="dashboard-grid">
            <div className="today-main-column">
                <section className="session-card">
                    ...
                    <div className="session-body"><div className="section-heading"><h2>The workout</h2><span className="muted">{preferences.weightUnit} · working sets</span></div>
                        ...
                        <div className="today-plan-list">{planned.map((prescription, index) => {
                            ...
                            return exercise && <div className="today-plan-row" key={prescription.exerciseId}>
            ...
            <aside className="side-column">
                <section className="mini-panel context-panel"><div className="panel-title"><span>Make it fit your day</span></div><p className="brief-note">{inProgress ? "Changes apply to your next session." : "Adjust these before you start."}</p>
                    ...
                    <details className="equipment-disclosure"><summary>Unavailable equipment{context.unavailableEquipment.length > 0 ? ` (${context.unavailableEquipment.length})` : ""}</summary><fieldset className="equipment-options"><legend className="sr-only">Equipment unavailable today</legend>{gym?.equipment.map((equipment) => <label key={equipment}><input type="checkbox" checked={context.unavailableEquipment.includes(equipment)} onChange={() => onContextChange({ ...context, unavailableEquipment: context.unavailableEquipment.includes(equipment) ? context.unavailableEquipment.filter((item) => item !== equipment) : [...context.unavailableEquipment, equipment] })} />{equipment.replaceAll("-", " ")}</label>)}</fieldset></details>
                </section>
