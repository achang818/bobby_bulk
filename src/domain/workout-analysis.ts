...
function describeExecution(outcome: SessionExerciseAssessment, session: WorkoutSession): ExerciseOutcome {
  ...
  if (status === 'skipped') observations.push(session.exerciseSwaps?.some((swap) => swap.fromExerciseId === outcome.exerciseId) ? 'You switched this movement. No sets were logged for the original exercise.' : session.exerciseOmissions?.find((item) => item.exerciseId === outcome.exerciseId)?.reason === 'voluntary' ? 'You recorded a voluntary skip. One choice does not establish an exercise preference.' : 'No prescribed sets logged. Missing work alone does not imply dislike or a performance decline.')
