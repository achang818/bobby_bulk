...
import { loadPlans, loadPreferences, loadSavedTemplates, loadWorkouts, loadGyms, loadRecommendationDecisions, latestRecommendationDecision, loadTodaysContext, deletePlan, deleteWorkout, savePlan, savePreferences, saveRecommendationDecision, saveWorkout, saveTodaysContext, toggleSavedTemplate, } from "./domain/storage";
import { applyAcceptedRecommendation } from "./domain/plan-actions";
import { completeWorkoutSession, createPlannedExercise, createWorkoutSession } from "./domain/workout-session";
...
function App() {
    ...
    const workoutsInCurrentUnit = useMemo(() => workouts.map((workout) => ({ ...workout, unit: preferences.weightUnit, sets: workout.sets.map((set) => ({ ...set, weight: displayWeight(set.weight, workout.unit, preferences.weightUnit), })), })), [preferences.weightUnit, workouts]);
    ...
    const adaptedPlanExerciseIds = useMemo(() => activePlan.exerciseIds.map((id) => {
        const recommendation = planRecommendations.find((item) => item.type === "REPLACE" && item.exerciseId === id && item.alternativeExerciseId && (item.trace.ruleId === "adapt-unavailable-equipment" || latestRecommendationDecision(item.id, recommendationDecisions) === "accepted"));
    ...
    const sessionPlannedExercises = useMemo(() => workoutExerciseIds.map((id, order) => {
        const exercise = exercises.find((item) => item.id === id);
        const original = activePlan.plannedExercises?.find((item) => item.exerciseId === id);
        return { ...(original ?? createPlannedExercise(id, order, exercise)), exerciseId: id, order, sets: setTargets.get(id) ?? exercise?.defaultSets ?? 0 };
    }), [activePlan.plannedExercises, setTargets, workoutExerciseIds]);
    ...
    const initialSetInput = useMemo<SetInput>(() => {
        ...
        return { weight: String(progression?.weight || latestSet?.weight || 0), reps: String(progression?.repRange.min ?? latestSet?.reps ?? exercise.repRange.min), rir: "", };
    ...
    function addSet() {
        ...
        if (!Number.isFinite(parsedWeight) || !Number.isFinite(parsedReps) || parsedReps <= 0 || (parsedRir !== undefined && (!Number.isFinite(parsedRir) || parsedRir < 0 || parsedRir > 5)))
            return;
        ...
        updateSessionSets((current) => [...current, { id: crypto.randomUUID(), exerciseId: selectedExerciseId, setType, weight: parsedWeight, reps: parsedReps, ...(parsedRir === undefined ? {} : { rir: parsedRir }), },]);
    ...
    function startPlan(plan: WorkoutTemplate) {
        ...
        const shortenedExerciseIds = plan.exerciseIds.map((id) => contextRecommendations.find((item) => item.type === "REPLACE" && item.exerciseId === id)?.alternativeExerciseId ?? id).filter((id) => !adaptWorkoutForTime(plan, exercises, todaysContext.availableMinutes, planGoalCriticalIds).some((item) => item.type === "REMOVE" && item.exerciseId === id));
        setActivePlan(plan);
    ...
    return (<div className="app-shell">      <header className="topbar">        <a className="brand" href="#top" onClick={() => setView("today")}>          <span className="brand-mark">B</span>          <span>            <strong>Bobby</strong>            <small>training log</small>          </span>        </a>        <nav className="main-nav" aria-label="Main navigation">          {(["today", "history", "exercises", "plans", "goals"] as View[]).map((item) => (<button key={item} className={view === item ? "nav-link active" : "nav-link"} onClick={() => setView(item)}>                {item === "today" ? "Today" : item === "history" ? "History" : item === "exercises" ? "Exercises" : item === "plans" ? "Plans" : "Goals"}              </button>))}        </nav>        <div className="profile-chip">          <div className="unit-toggle">            <button className={preferences.weightUnit === "kg" ? "selected" : ""} onClick={() => setPreferences(savePreferences({ ...preferences, weightUnit: "kg" }))}>              kg            </button>            <button className={preferences.weightUnit === "lb" ? "selected" : ""} onClick={() => setPreferences(savePreferences({ ...preferences, weightUnit: "lb" }))}>              lb            </button>          </div>          <span className="profile-dot">A</span>          <span>Alex</span>        </div>      </header>      <main id="top">        {view === "today" && (<EvaluatedTodayView plan={activePlan} exerciseIds={workoutExerciseIds} setTargets={setTargets} recommendations={planRecommendations} decisions={recommendationDecisions} workouts={workoutsInCurrentUnit} unit={preferences.weightUnit} gyms={gyms} todaysContext={todaysContext} fatigue={classifyFatigue(workouts)} onContextChange={updateTodaysContext} onLog={startRecommendedWorkout} onChooseSomethingElse={() => setView("plans")} onDecision={handleRecommendationDecision}/>)}        {view === "history" && (<HistoryView workouts={workouts} unit={preferences.weightUnit} onDelete={(workoutId) => setWorkouts(deleteWorkout(workoutId, workouts))}/>)}        {view === "exercises" && <ExercisesView />}        {view === "plans" && (<PlansView plans={plans} onSave={(plan) => setPlans(savePlan(plan))} onStart={startPlan} onDelete={(planId) => setPlans(deletePlan(planId))}/>)}        {view === "goals" && (<GoalsView preferences={preferences} onSave={(next) => {
                ...
                if (!exercise)
                    return null;
                return (<article className={selectedExerciseId === id ? "logger-exercise active" : "logger-exercise"} key={id}>                    <button className="logger-exercise-heading" onClick={() => setSelectedExerciseId(id)}>                      <span className="logger-exercise-number">{String(index + 1).padStart(2, "0")}</span>                      <span>                        <strong>{exercise.name}</strong>                        <small>{loggedSets.length}/{loggerSetTargets.get(id) ?? exercise.defaultSets} planned {plannedExercise?.setType ?? "working"} sets</small>                      </span>                      <span className="logger-exercise-state">{loggedSets.length === (loggerSetTargets.get(id) ?? exercise.defaultSets) ? "✓" : ""}</span>                    </button>                    {selectedExerciseId === id && <div className="logger-set-entry">                      {loggedSets.map((set, setIndex) => <div className="logged-set" key={set.id}><span>Set {setIndex + 1}</span><strong>{set.weight} {preferences.weightUnit} × {set.reps}</strong><span>RIR {set.rir ?? "-"}</span></div>)}                      <div className="input-row">                        <label>                          Weight                          <input inputMode="decimal" value={setInput.weight} onChange={(event) => updateSetInput({ weight: event.target.value })}/>                        </label>                        <label>                          Reps                          <input inputMode="numeric" value={setInput.reps} onChange={(event) => updateSetInput({ reps: event.target.value })}/>                        </label>                        <label>                          RIR                          <input inputMode="numeric" min="0" max="5" placeholder="Optional" value={setInput.rir} onChange={(event) => updateSetInput({ rir: event.target.value })}/>                        </label>                      </div>                      <button className="secondary-button full" onClick={addSet}>                        {loggedSets.length >= (loggerSetTargets.get(id) ?? exercise.defaultSets) ? "＋ Log extra working set" : "✓ Log working set"}                      </button>                      {loggedSets.length >= (loggerSetTargets.get(id) ?? exercise.defaultSets) && (<p className="extra-set-note">The recommendation is complete. Extra sets are recorded and included in your history.</p>)}                    </div>}                  </article>);
...
function EvaluatedTodayView({ plan, exerciseIds, setTargets, recommendations, decisions, workouts, unit, gyms, todaysContext, fatigue, onContextChange, onLog, onChooseSomethingElse, onDecision, }: {
    ...
    return (<>      <section className="page-intro">        <div>          <p className="eyebrow">{new Date().toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}</p>          <h1>Today's Recommendation</h1>          <p className="lede">            A focused session based on your plan, recent training, and performance.          </p>        </div>        <button className="primary-button" onClick={hasWorkout ? onLog : onChooseSomethingElse}>          {hasWorkout ? "Start workout" : "Create a plan"}        </button>      </section>      <section className="dashboard-grid">        <div className="recommendation-panel">          <div className="panel-topline">            <span className="status-dot"></span>            <span>Recommended session</span>            <span className="confidence">              {recommendations.length} observations            </span>            <span className="fatigue-indicator">Fatigue {fatigue}</span>          </div>          <div className="recommendation-main">            <div>              <p className="eyebrow warm">YOUR NEXT WORKOUT · LOADS IN {unit}</p>              <h2>{plan.name}</h2>              <p className="recommendation-sub">                Based on your plan, recent training, and performance.              </p>            </div>            <div className="weight-callout">              <strong>{exerciseIds.length}</strong>              <span>movements</span>            </div>          </div>          <div className="today-plan-list">            <div className="today-plan-label">Recommended working sets</div>            {!hasWorkout && <p className="empty-state">Create your first plan to give Bobby a routine to adapt and evaluate.</p>}            {exerciseIds.map((id, index) => {
            ...
            return exercise ? (<div className="today-plan-row" key={id}>                  <span>{String(index + 1).padStart(2, "0")}</span>                  <div>                    <strong>{exercise.name}</strong>                    <small>{setTargets.get(id) ?? exercise.defaultSets} sets · {exercise.repRange.min}–{exercise.repRange.max} reps</small>                  </div>                  <small className="today-load">{progression?.weight ? `${progression.weight} ${unit}` : "Use last session"}</small>                </div>) : null;
