...
const emptyPlan: WorkoutTemplate = {
  id: "no-plan-selected",
  name: "No workout selected",
  description: "Create a plan first, then Bobby can recommend how to perform it today.",
  focus: "Your training plan",
  exerciseIds: [],
};
...
function App() {
  ...
  const workoutsInCurrentUnit = useMemo(() => workouts.map((workout) => ({
    ...workout,
  ...
  })), [preferences.weightUnit, workouts]);
  const goalCriticalExerciseIds = activePlan.exerciseIds.filter((id) => {
  ...
  function addSet() {
    ...
    if (
      ...
      return;
    const target = setTargets.get(selectedExerciseId) ?? 0;
    const completedSets = draftSets.filter((set) => set.exerciseId === selectedExerciseId).length;
    ...
    if (completedSets + 1 === target) {
      ...
      const nextExerciseId = workoutExerciseIds.slice(currentIndex + 1).find((id) =>
        draftSets.filter((set) => set.exerciseId === id).length < (setTargets.get(id) ?? 0),
      );
      if (nextExerciseId) setSelectedExerciseId(nextExerciseId);
    }
  }
  ...
  function startPlan(plan: WorkoutTemplate) {
    const planGoalCriticalIds = plan.exerciseIds.filter((id) => {
      const exercise = exercises.find((item) => item.id === id);
      return exercise?.primaryMuscles.some((muscle) => preferences.priorities.some((priority) => priority.toLowerCase() === muscle.toLowerCase())) ?? false;
    });
    const contextRecommendations = adaptWorkout(plan, exercises, todaysContext, preferences);
    const shortenedExerciseIds = plan.exerciseIds
      .map((id) => contextRecommendations.find((item) => item.type === "REPLACE" && item.exerciseId === id)?.alternativeExerciseId ?? id)
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <a className="brand" href="#top" onClick={() => setView("today")}>
        ...
        <nav className="main-nav" aria-label="Main navigation">
          {(["today", "history", "exercises", "plans", "goals"] as View[]).map(
            (item) => (
              <button
                key={item}
        ...
        <div className="profile-chip">
          <div className="unit-toggle">
            <button
              className={preferences.weightUnit === "kg" ? "selected" : ""}
              onClick={() =>
                setPreferences(
                  savePreferences({ ...preferences, weightUnit: "kg" }),
                )
              }
            >
              kg
            ...
            <button
              className={preferences.weightUnit === "lb" ? "selected" : ""}
              onClick={() =>
                setPreferences(
                  savePreferences({ ...preferences, weightUnit: "lb" }),
                )
              }
            >
              lb
      ...
      <main id="top">
        {view === "today" && (
          <EvaluatedTodayView
            ...
            workouts={workoutsInCurrentUnit}
            unit={preferences.weightUnit}
            gyms={gyms}
        ...
        {view === "history" && (
          <HistoryView
            workouts={workouts}
            unit={preferences.weightUnit}
            onDelete={(workoutId) => setWorkouts(deleteWorkout(workoutId, workouts))}
          />
        )}
        ...
        {view === "plans" && (
          <PlansView
            plans={plans}
            onSave={(plan) => setPlans(savePlan(plan))}
            onStart={startPlan}
            onDelete={(planId) => setPlans(deletePlan(planId))}
          />
        )}
        {view === "goals" && (
          <GoalsView
      ...
      {showLogger && (
        <div
          ...
          role="presentation"
          onMouseDown={(event) =>
            event.target === event.currentTarget && setShowLogger(false)
          }
        >
          <section className="modal" aria-labelledby="logger-title">
            <div className="modal-heading">
              <div>
                <p className="eyebrow">{activePlan.name}</p>
                <h2 id="logger-title">Log workout</h2>
              </div>
              <button
            ...
            <div className="plan-preview">
              {workoutExerciseIds.map((id, index) => (
                <span
                  key={id}
                  className={
                    id === selectedExerciseId
                      ? "plan-exercise active"
                      : "plan-exercise"
                  }
                >
                  {index + 1}.{" "}
                  ...
                  <article className={selectedExerciseId === id ? "logger-exercise active" : "logger-exercise"} key={id}>
                    <button className="logger-exercise-heading" onClick={() => setSelectedExerciseId(id)}>
                      ...
                      <span>
                        ...
                        <small>{loggedSets.length}/{setTargets.get(id) ?? exercise.defaultSets} recommended working sets · {exercise.repRange.min}–{exercise.repRange.max} reps</small>
                      </span>
                      <span className="logger-exercise-state">{loggedSets.length === (setTargets.get(id) ?? exercise.defaultSets) ? "✓" : ""}</span>
                    </button>
                    {selectedExerciseId === id && <div className="logger-set-entry">
                      ...
                      <button className="secondary-button full" onClick={addSet}>
                        {loggedSets.length >= (setTargets.get(id) ?? exercise.defaultSets) ? "＋ Log extra working set" : "✓ Log working set"}
                      </button>
                      {loggedSets.length >= (setTargets.get(id) ?? exercise.defaultSets) && (
                        <p className="extra-set-note">The recommendation is complete. Extra sets are recorded and included in your history.</p>
                      )}
                    </div>}
                  </article>
                );
            ...
            <div className="draft-list">
              {draftSets.length === 0 ? (
                <p className="empty-state">
                  Log sets as you move through the plan.
                </p>
              ) : (
                draftSets.map((set) => (
                  <div className="draft-set" key={set.id}>
                    <span>
                      {
                        exercises.find(
                      ...
                      · Set {draftSets.filter((item) => item.exerciseId === set.exerciseId).findIndex((item) => item.id === set.id) + 1}
                    </span>
                    <strong>
                      {set.weight} {preferences.weightUnit} × {set.reps}
                    </strong>
                    <button
                      onClick={() =>
                        setDraftSets((current) =>
            ...
            <button
              ...
              disabled={draftSets.length === 0}
              onClick={finishWorkout}
            >
              Finish workout
            </button>
            <p className="timing-note">
              Timing is optional. You can add set and rest tracking later
...
export function LegacyTodayView({
  plan,
  workouts,
  onLog,
}: {
    ...
    <>
      ...
      <section className="dashboard-grid">
        <div className="recommendation-panel">
          <div className="panel-topline">
            ...
            <span>Recommended workout</span>
            <span className="confidence">Based on your history</span>
          </div>
          <div className="recommendation-main">
            <div>
              <p className="eyebrow warm">NEXT SESSION</p>
              <h2>{plan.name}</h2>
              <p className="recommendation-sub">
                {plan.focus} · {plan.exerciseIds.length} exercises
              </p>
            </div>
            ...
            {plan.exerciseIds.map((id, index) => {
              ...
              return exercise ? (
                <div className="plan-row" key={id}>
                  ...
                  <strong>{exercise.name}</strong>
                  <small>
                    {exercise.defaultSets} × {exercise.repRange.min}–
          ...
          <div className="why-box" id="recommendation-changes">
            ...
            <p>
              <span>↳</span>Your last logged work informs progression inside
              each movement.
            </p>
            <p>
              <span>↳</span>Keep the session flexible if recovery or equipment
              changes.
            </p>
          </div>
          <div className="recommendation-actions">
            ...
            <button className="text-button">Change plan →</button>
          </div>
        </div>
        <aside className="side-column">
          ...
          <div className="mini-panel last-session">
            <div className="panel-title">
              ...
              <span>{lastWorkout ? formatDate(lastWorkout.date) : "—"}</span>
            </div>
            <strong>{lastWorkout?.title ?? "No sessions yet"}</strong>
            <p>
              {lastWorkout
                ? `${lastWorkout.sets.length} sets logged`
      ...
      <section className="focus-strip">
        ...
        <div className="focus-stats">
          <span>
            <strong>2</strong> workouts
          </span>
          <span>
            <strong>6</strong> exercises
          </span>
          <span>
    ...
    <>
      <section className="page-intro">
        <div>
          ...
          <p className="lede">
            A focused session based on your plan, recent training, and performance.
          </p>
        </div>
        <button className="primary-button" onClick={hasWorkout ? onLog : onChooseSomethingElse}>
          {hasWorkout ? "Start workout" : "Create a plan"}
        </button>
      </section>
      <section className="dashboard-grid">
        <div className="recommendation-panel">
          ...
          <div className="recommendation-main">
            <div>
              <p className="eyebrow warm">YOUR NEXT WORKOUT · LOADS IN {unit}</p>
              <h2>{plan.name}</h2>
              <p className="recommendation-sub">
                Based on your plan, recent training, and performance.
              </p>
            </div>
            <div className="weight-callout">
              <strong>{exerciseIds.length}</strong>
              <span>movements</span>
            </div>
          </div>
          <div className="today-plan-list">
            <div className="today-plan-label">Recommended working sets</div>
            {!hasWorkout && <p className="empty-state">Create your first plan to give Bobby a routine to adapt and evaluate.</p>}
            {exerciseIds.map((id, index) => {
              const exercise = exercises.find((item) => item.id === id);
              const progression = recommendations.find((item) => item.type === "PROGRESSION" && item.exerciseId === id)?.progression;
              return exercise ? (
                <div className="today-plan-row" key={id}>
                  <span>{String(index + 1).padStart(2, "0")}</span>
                  <div>
                    <strong>{exercise.name}</strong>
                    <small>{setTargets.get(id) ?? exercise.defaultSets} sets · {exercise.repRange.min}–{exercise.repRange.max} reps</small>
                  </div>
          ...
          <details className="context-controls">
            <summary>
              ...
              <small>{gyms.find((gym) => gym.id === todaysContext.gymId)?.name ?? "Gym not set"}{todaysContext.availableMinutes ? ` · ${todaysContext.availableMinutes} min` : ""}</small>
            </summary>
            <p className="context-help">Temporary constraints adapt today’s workout only; your saved plan stays unchanged.</p>
          ...
          <div className="why-box" id="recommendation-changes">
            ...
            {changes.length === 0 ? (
              <p>
                <span>↳</span>No changes are strongly supported by the current
                history. Keep following your plan.
              </p>
            ) : (
              changes
                .map((item) => (
          ...
          <div className="recommendation-actions">
            ...
            <button className="text-button" onClick={onChooseSomethingElse}>Choose something else</button>
          </div>
        </div>
        <aside className="side-column">
          <div className="mini-panel session-brief">
            ...
            <div className="brief-row">
              ...
              <strong>{exerciseIds.reduce((total, id) => total + (setTargets.get(id) ?? exercises.find((exercise) => exercise.id === id)?.defaultSets ?? 0), 0)}</strong>
            </div>
            <div className="brief-row">
              <span>Gym</span>
            ...
            <div className="brief-row">
              ...
              <strong className={`fatigue-${fatigue.toLowerCase()}`}>{fatigue}</strong>
            </div>
...
function RecommendationRow({
  ...
  const exercise = exercises.find(
    (item) => item.id === recommendation.exerciseId,
  );
  const alternative = recommendation.alternativeExerciseId
    ? exercises.find((item) => item.id === recommendation.alternativeExerciseId)
    : undefined;
    ...
    <div className="recommendation-row">
      <div>
        ...
        {recommendation.progression && (
          <small>
            {recommendation.progression.weight
              ? `${recommendation.progression.weight} ${unit} · ${recommendation.progression.sets} × ${recommendation.progression.repRange.min}–${recommendation.progression.repRange.max}`
        ...
        <details className="recommendation-details">
          ...
          <small>{recommendation.trace.principleDescription}</small>
          <small className="evidence-badge">
            Evidence {recommendation.trace.evidenceLevel} ·{" "}
            {recommendation.trace.source.name}
          </small>
        </details>
      </div>
      <div className="recommendation-controls">
        {requiredToday ? (
          <span className="recommendation-status applied">Applied today</span>
        ) : decision === "accepted" ? (
          <>
    ...
    <>
      <section className="page-intro compact">
        <div>
          ...
          <p className="lede">
            Keep it simple. Bobby uses these priorities to evaluate your
            existing plans.
          </p>
        </div>
      </section>
      <section className="goals-layout">
        <div className="goals-panel">
          <p className="eyebrow">Primary goals</p>
          <div className="goal-options">
            {goalOptions.map((goal) => (
              <button
                key={goal}
                className={
                  goals.includes(goal) ? "goal-option active" : "goal-option"
                }
          ...
          <p className="eyebrow priority-label">
            Anything you particularly want to improve?
          </p>
          ...
          <button
            className="primary-button save-goals"
            onClick={() => onSave({ ...preferences, goals, priorities })}
          >
            Save preferences
          </button>
        </div>
        <aside className="mini-panel goals-note">
          <span className="why-icon">i</span>
          <h2>How this helps</h2>
          <p>
            Your choices become structured evidence for plan evaluation. They
            can suggest an addition or support keeping an exercise, but Bobby
            will not change your plan automatically.
          </p>
        </aside>
      </section>
    </>
...
function HistoryView({
  ...
  const exerciseSets = workouts.flatMap((workout) =>
    workout.sets
      ...
      .map((set) => ({
        ...set,
        weight: displayWeight(set.weight, workout.unit, unit),
        date: workout.date,
      })),
  );
  return (
    <>
      <section className="page-intro compact">
        <div>
          <p className="eyebrow">Your training archive</p>
          <h1>History</h1>
          <p className="lede">
            Every session is evidence. Browse the work, then spot the trend.
          </p>
        </div>
        <span className="archive-count">{workouts.length} sessions</span>
      </section>
      <div className="history-layout">
          ...
          {workouts.map((workout) => (
            <article className="workout-row" key={workout.id}>
              ...
              <div className="workout-info">
                ...
                <span>
                  {new Set(workout.sets.map((set) => set.exerciseId)).size}{" "}
        ...
        <aside className="progression-detail">
          <div className="section-heading">
          ...
          <select
            value={selectedExerciseId ?? ""}
            onChange={(event) =>
              setSelectedExerciseId(event.target.value || null)
            }
          >
          ...
          {selected ? (
            <div className="progression-content">
              ...
              <div className="progression-stat">
                <strong>
                  {exerciseSets.length ? exerciseSets[0].weight : 0}
                </strong>
                <span>{unit} latest load</span>
              </div>
              {exerciseSets.slice(0, 8).map((set) => (
                <div className="progression-set" key={`${set.id}-${set.date}`}>
            ...
            <p className="empty-state">
              Choose an exercise to see its set-by-set progression.
            </p>
          )}
        </aside>
      </div>
...
function ExercisesView() {
  ...
  const filteredExercises = exercises.filter((exercise) => {
    const haystack =
      `${exercise.name} ${exercise.category} ${exercise.equipment} ${exercise.primaryMuscles.join(" ")}`.toLowerCase();
    return (
    ...
    <>
      <section className="page-intro compact">
        <div>
          ...
          <h1>Every movement, in one place.</h1>
          <p className="lede">
            Search by movement, muscle, or equipment. Open an exercise to
            inspect its training context.
          </p>
        </div>
      </section>
        ...
        <div className="filter-row">
          {categories.map((category) => (
            <button
              key={category}
              className={
                filter === category ? "filter-chip active" : "filter-chip"
              }
      ...
      <div className="result-line">
        <span>{filteredExercises.length} movements</span>
      ...
      <div className="exercise-library">
        {filteredExercises.map((exercise) => {
          ...
          return (
            <article
              className={expanded ? "library-card expanded" : "library-card"}
              key={exercise.id}
            >
              <button
                className="exercise-row-trigger"
                ...
                <div className="exercise-details">
                  ...
                  <div className="detail-grid">
                    <div>
                      ...
                      <strong>{exercise.goals.join(" · ")}</strong>
                    </div>
                    <div>
                      <span className="detail-label">Working range</span>
                      <strong>
                        {exercise.defaultSets} sets · {exercise.repRange.min}–
                        {exercise.repRange.max} reps
                      </strong>
                    </div>
                    <div>
        ...
        {filteredExercises.length === 0 && (
          <div className="no-results">
            ...
            <span>Try a different name, muscle, or equipment.</span>
          </div>
        )}
      </div>
    </>
...
export function LegacyExercisesView({
  onSelect,
}: {
  ...
  const [savedIds, setSavedIds] = useState(
    () => new Set(loadSavedTemplates().map((template) => template.id)),
  );
  const categories = [
    "All",
    ...new Set(exercises.map((exercise) => exercise.category)),
    ...
    <>
      <section className="page-intro compact">
        <div>
          ...
          <p className="lede">
            Find a movement or start with a ready-made plan.
          </p>
        </div>
      </section>
      <div className="library-tabs" role="tablist">
        <button
          className={
            section === "exercises" ? "library-tab active" : "library-tab"
          }
          onClick={() => setSection("exercises")}
        >
          Exercises <span>{exercises.length}</span>
        </button>
        <button
          className={
            section === "workouts" ? "library-tab active" : "library-tab"
          }
          onClick={() => setSection("workouts")}
        >
          Premade workouts <span>{workoutTemplates.length}</span>
        </button>
      </div>
      {section === "exercises" ? (
        <>
          ...
          <div className="result-line">
            <span>{filteredExercises.length} movements</span>
            <span>Tap a movement to use it today</span>
          </div>
          <div className="exercise-library">
            {filteredExercises.map((exercise) => (
          ...
          {workoutTemplates.map((template) => (
            <article className="template-card" key={template.id}>
              <div className="template-art">
                ...
                <small>{template.exerciseIds.length} exercises</small>
              </div>
              <div className="template-body">
                <div className="card-kicker">{template.focus}</div>
                <h2>{template.name}</h2>
                <p>{template.description}</p>
                <div className="template-exercises">
                  {template.exerciseIds.slice(0, 4).map((id) => (
                    <span key={id}>
                      {exercises.find((exercise) => exercise.id === id)?.name}
                    </span>
                  ))}
...
function PlansView({
  ...
  function createPlan() {
    ...
    onSave({
      ...
      name: name.trim(),
      description: "A plan built around your current priorities.",
      focus: focus.trim() || "Personal plan",
  ...
  const allPlans = [
    ...
    ...workoutTemplates.filter(
      (template) => !plans.some((plan) => plan.id === template.id),
    ),
  ];
  return (
    <>
      <section className="page-intro compact">
        <div>
          <p className="eyebrow">Your training system</p>
          <h1>Plans</h1>
          <p className="lede">
            Create the days you want to repeat. Bobby will keep the history and
        ...
        <section className="plan-builder">
          <div className="section-heading">
            <div>
              ...
              <h2>Build your own day</h2>
            </div>
            <span className="plan-count">{selectedIds.length} exercises</span>
          </div>
          <div className="input-row">
            <label>
              Plan name
          ...
          <button className="secondary-button" onClick={addExercise}>
            ＋ Add to plan
          </button>
          <div className="builder-list">
            {selectedIds.length === 0 ? (
              <p className="empty-state">
                Your plan is empty. Add the movements you want to train.
              </p>
            ) : (
              selectedIds.map((id, index) => {
                const exercise = exercises.find((item) => item.id === id);
                return exercise ? (
                  <div className="builder-row" key={id}>
                    <span>{String(index + 1).padStart(2, "0")}</span>
                    <strong>{exercise.name}</strong>
                    <small>
                      {exercise.defaultSets} sets · {exercise.repRange.min}–
          ...
          {allPlans.map((plan) => (
            <article className="saved-plan" key={plan.id}>
              ...
              <div className="saved-plan-actions">
                <button className="text-button" onClick={() => onStart(plan)}>
                  Start →
                </button>
                {plans.some((item) => item.id === plan.id) && (
                  <button
                    className="delete-button"
                    onClick={() => {
                      if (window.confirm(`Delete ${plan.name}?`)) onDelete(plan.id);
                    }}
